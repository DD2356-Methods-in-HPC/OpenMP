...
int main(int argc, char *argv[]) {
  ...
  if (argc > 1) {
    // indexed in zero => get argument[1]
...
int DFT(int idft, double *xr, double *xi, double *Xr_o, double *Xi_o, int N) {
  // parallelizing with OpenMP, each thread has a copy of k and n => private(k, n)
  // the rest are shared variables
  #pragma omp parallel for shared(xr, xi, Xr_o, Xi_o, N, idft) schedule(guided)
  for (int k = 0; k < N; k++) {
    for (int n = 0; n < N; n++) {
      // Real part of X[k]
      Xr_o[k] +=
